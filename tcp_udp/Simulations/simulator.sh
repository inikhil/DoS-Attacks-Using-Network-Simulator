...
for i in `seq 1 100`;
    do
		j=$(($i * 5))
		k="$i kb"
        ns attack3.tcl "$i.tr" "$i.out" $j

		#awk -f systhroughput.awk $i.tr > throughput.$i
		#awk -f drop.awk $i.tr > prdrop.$i
		#awk -f packetdrop.awk $i.tr > pacdrop.$i
		#awk -f queuemonitor.awk $i.out > queuelen.$i
		#awk -f waitqueue.awk $i.out > waittime.$i
		...
		cat throughput.$i >> throughput2.dat
		cat prdrop.$i >> prdrop2.dat
		cat pacdrop.$i >> pacdrop2.dat
		cat queuelen.$i >> queuelen2.dat
		cat waittime.$i >> waittime2.dat
